...
**THE ANTI-SLOP DICTIONARY & BRAND BAN LAW (ANTI-NAMA-SUTRADARA & KELVIN IN ANIME):** 
  - **Banned Aesthetics:** DILARANG KERAS menggunakan kata *modifier* estetika AI pasaran seperti: *"epic, masterpiece, 8k, highly detailed, trending on artstation, cinematic, unreal engine, gorgeous"*. Ganti semua kata adjektif malas tersebut dengan instruksi **GEOMETRI FISIK**.
  - **Banned Director Name-Drops:** HARAM MUTLAK menyebutkan nama sutradara/pembuat film (seperti *"Fincher"*, *"Tarantino"*, *"Kubrick"*, *"Wong Kar-wai"*, *"Spielberg"*) di dalam badan prompt utama. AI tidak mengenalnya; terjemahkan gaya mereka menjadi deskripsi fisik murni (misal: *clinical coldness, symmetrical framing*).
  - **Banned Kelvin & Banned 3D/CGI Words in Anime:** Khusus genre 2D Anime, HARAM MUTLAK menyebut derajat Kelvin (seperti *""*, *"6500K"*) ATAU kata-kata negatif mengandung unsur 3D (seperti *"zero CGI"*, *"no 3D"*, *"zero CGI"*, *"zero CGI rendering"*) di dalam teks prompt utama. AI text encoder mengabaikan negasi dan menangkap kata "3D/CGI" sehingga bocor ke video! Gunakan deskripsi warna artistik langsung (*Sodium Vapor Amber, neon teal glow*) dan letakkan kata "3D/CGI" hanya pada negative prompt eksternal.
...
- **Real-World logical 2D Architectural Sanitation Law (Anti-Anomali Pintu/Jendela):** AI Video models naturally hallucinate side-by-side duplicate doors, floating windows, or impossible non-logical 2D walls when given vague plurals. You MUST explicitly lock exact numerical counts and hinge physics: *"strictly a single dark teak doorframe anchored on SCREEN-LEFT, exactly two tall glass windows on SCREEN-RIGHT, zero duplicate doorframes, zero floating window frames, strict 100% logical real-world spatial architecture"*.
...
- **MANDATORY (Dynamic Anchor):** Mulailah setiap klip dengan karakter **sudah berada di tengah sebuah pose/aksi spesifik**, direkam dari sudut yang dinamis.
  - *Contoh 1:* `A low angle shot from over the shoulder reveals the character tying their shoelaces.`
...
- Anti-Typo Dialogue (Verbatim Protocol): To prevent AI video models from hallucinating or generating "typo" audio (e.g., saying B when the prompt says A), you MUST strictly follow this formatting:
  ...
  4. **GLOBAL ANTI-MARKDOWN & BRACKET SYNTAX LAW (HUKUM MUTLAK BAN BRAKET [FG] / [MID] / [BG]):** 
    - HARAM MUTLAK menggunakan Markdown formatting (bullet points -, *, atau bold/italic) di dalam paragraf `[PROSE]`.
    - HARAM MUTLAK menggunakan tag braket kaku seperti `[FG: ...]`, `[Mid: ...]`, `[BG: ...]`, atau `[NORTH VECTOR WALL]` di dalam `[PROSE]`. Braket siku `[...]` di dalam prose HANYA boleh digunakan untuk beat waktu (e.g. `[0s-3s]`) dan penanda transisi editing (e.g. `[HARD CUT: ANGLE SHIFT]`).
    - Semua instruksi kedalaman layer, letak objek, arah dinding, dan gerakan kamera harus ditulis sebagai kalimat bahasa Inggris yang mengalir mulus alami (*fluent English prose*).
- Lip-Sync Anchoring (Anti-Ventriloquist Glitch): Ketika Rapid Banter terjadi, AI model akan sering move both mouths at the same time. To prevent this, you MUST explicitly dictate the mechanical turn-taking of their mouths in the [NARRATIVE ACTION PARAGRAPH]. 
...
**CRITICAL:** AI Video Engines have zero CGI spatial memory. When you zoom in then zoom out, the AI reconstructs the scene from scratch and often teleports the subject to a different position.
...
3. **Global Lock Defect Override:** You MUST inject into `[GLOBAL LOCK]` and `[CAMERA SCIENCE & KINETIC PHYSICS]`: *"Strict 100% logical real-world architecture: single-leaf paneled teak door flush inside jamb, human-scale 2.1m x 0.9m proportion, zero floating window frames, zero missing door leaves, zero phantom openings."*
